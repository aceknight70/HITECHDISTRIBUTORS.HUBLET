...
export default function App() {
  ...
  const [solarProductsList, setSolarProductsList] = useState<SolarProduct[]>(() => {
    try {
      const stored = customStorage.getItem('ht_solar_products');
  ...
  const handleSaveProduct = async (prod: Product) => {
    ...
    try {
      customStorage.setItem('ht_products', JSON.stringify(nextList));
  ...
  const handleSaveSolarProduct = async (solar: SolarProduct) => {
    ...
    try {
      customStorage.setItem('ht_solar_products', JSON.stringify(nextList));
  ...
  const handleSaveGalleryPhoto = async (updatedPhoto: any) => {
    ...
    customStorage.setItem('ht_gallery_photos', JSON.stringify(nextPhotos));
  ...
  const [isStaffLoggedIn, setIsStaffLoggedIn] = useState(() => {
    try {
      return customStorage.getItem('ht_staff_login') === 'true';
    } catch {
      return false;
    }
  });
  const [staffRole, setStaffRole] = useState(() => {
    try {
      return customStorage.getItem('ht_staff_role') || 'staff';
    } catch {
      return 'staff';
    }
  });
  ...
  useEffect(() => {
    const handleSync = () => {
      try {
        const logged = customStorage.getItem('ht_staff_login') === 'true';
        const r = customStorage.getItem('ht_staff_role') || 'staff';
    ...
    try {
      unsubP = onSnapshot(collection(db, 'products'), (snapshot) => {
        ...
        if (list.length > 0) {
          ...
          customStorage.setItem('ht_products', JSON.stringify(list));
        } else {
      ...
      unsubS = onSnapshot(collection(db, 'solar_products'), (snapshot) => {
        ...
        if (list.length > 0) {
          ...
          customStorage.setItem('ht_solar_products', JSON.stringify(list));
        } else {
  ...
  const [openingPhotoUrl, setOpeningPhotoUrl] = useState(() => {
    return customStorage.getItem('ht_opening_photo') || 'https://images.unsplash.com/photo-1542751371-adc38448a05e?w=1600&q=80';
  });
  
  const [liveEmbedUrl, setLiveEmbedUrl] = useState(() => {
    return customStorage.getItem('ht_live_embed') || 'https://www.youtube.com/embed/live_stream?channel=UC...';
  });

  const [contacts, setContacts] = useState(() => {
    try {
      const stored = customStorage.getItem('ht_contacts');
      if (stored) {
  ...
  const [requestsList, setRequestsList] = useState<any[]>(() => {
    try {
      const stored = customStorage.getItem('ht_requests');
  ...
  const [galleryVideos, setGalleryVideos] = useState<any[]>(() => {
    try {
      const stored = customStorage.getItem('ht_videos');
      return stored ? JSON.parse(stored) : [];
    } catch {
      return [];
    }
  ...
  const [displayFloorActiveLayout, setDisplayFloorActiveLayout] = useState<string>(() => {
    return customStorage.getItem('ht_display_floor_active') || '1, 2, 3, 4, 14, 25, 30, 42, 44, 45, 46, 47, 48, 49, 55, 56, 58';
  });

  const [displayFloorSavedConfigs, setDisplayFloorSavedConfigs] = useState<{ [name: string]: string }>(() => {
    try {
      const stored = customStorage.getItem('ht_display_floor_saved');
  ...
  const [spreadsheetId, setSpreadsheetId] = useState<string>(() => {
    return customStorage.getItem('ht_spreadsheet_id') || '';
  });

  const handleUpdateSpreadsheetId = (id: string) => {
    setSpreadsheetId(id);
    customStorage.setItem('ht_spreadsheet_id', id);
  };
  ...
  const [spreadsheetHeaders, setSpreadsheetHeaders] = useState<string[]>(() => {
    try {
      const saved = customStorage.getItem('ht_spreadsheet_headers');
  ...
  const [spreadsheetRows, setSpreadsheetRows] = useState<string[][]>(() => {
    try {
      const saved = customStorage.getItem('ht_spreadsheet_rows');
      return saved ? JSON.parse(saved) : [
  ...
  const [imageCache, setImageCache] = useState<{ [key: string]: string }>(() => {
    try {
      const persistedI = customStorage.getItem('ht_img_cache');
  ...
  const [galleryPhotos, setGalleryPhotos] = useState<any[]>(() => {
    try {
      const persistedG = customStorage.getItem('ht_gallery_photos');
  ...
  const handleUpdateImageCache = (key: string, val: string) => {
    setImageCache(prev => {
      ...
      customStorage.setItem('ht_img_cache', JSON.stringify(next));
      return next;
    });
  };
  ...
  const saveGalleryPhotosToStorage = async (updatedPhotos: any[]) => {
    ...
    customStorage.setItem('ht_gallery_photos', JSON.stringify(formatted));
  ...
  const [csvUploadStatus, setCsvUploadStatus] = useState<string>(() => {
    return customStorage.getItem('ht_csv_upload_status') || 'No Google Sheets CSV imported yet';
  });
    ...
    try {
      ...
      if (parsedVideos.length > 0) {
        setGalleryVideos(prev => {
          ...
          customStorage.setItem('ht_videos', JSON.stringify(joined));
          return joined;
        });
      }
      ...
      customStorage.setItem('ht_csv_upload_status', statusText);
      
      if (allRows.length > 0) {
        setSpreadsheetHeaders(allRows[0]);
        setSpreadsheetRows(allRows.slice(1));
        customStorage.setItem('ht_spreadsheet_headers', JSON.stringify(allRows[0]));
        customStorage.setItem('ht_spreadsheet_rows', JSON.stringify(allRows.slice(1)));
        customStorage.setItem('ht_last_imported_headers', JSON.stringify(allRows[0]));
        customStorage.setItem('ht_last_imported_rows', JSON.stringify(allRows.slice(1)));
      }
  ...
  const handleSyncSpreadsheetToDatabase = async (customHeaders: string[], customRows: string[][], isSilent: boolean = false, skipLayoutUpdate: boolean = false) => {
    try {
      ...
      customStorage.setItem('ht_spreadsheet_headers', JSON.stringify(customHeaders));
      customStorage.setItem('ht_spreadsheet_rows', JSON.stringify(customRows));
  ...
  const handleFactoryResetDatabase = async () => {
    try {
      ...
      customStorage.removeItem('ht_products');
      customStorage.removeItem('ht_gallery_photos');
      customStorage.removeItem('ht_videos');
      customStorage.removeItem('ht_img_cache');
      
      setCsvUploadStatus('No Google Sheets CSV imported yet');
      customStorage.removeItem('ht_csv_upload_status');
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'reviews'), (snapshot) => {
      if (snapshot.empty) {
        ...
        customStorage.setItem('ht_rev', JSON.stringify(items));
      }
    }, (error) => {
      // Graceful local offline fallback
      try {
        const persistedRev = customStorage.getItem('ht_rev');
        if (persistedRev) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'repairs'), (snapshot) => {
      ...
      customStorage.setItem('ht_repairs', JSON.stringify(items));
    }, (error) => {
      // Graceful local offline fallback
      try {
        const persistedR = customStorage.getItem('ht_repairs');
        if (persistedR) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'escalations'), (snapshot) => {
      ...
      customStorage.setItem('ht_escalations', JSON.stringify(items));
    }, (error) => {
      try {
        const persisted = customStorage.getItem('ht_escalations');
        if (persisted) {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'manager_requests'), (snapshot) => {
      ...
      customStorage.setItem('ht_manager_requests', JSON.stringify(items));
    }, (error) => {
      try {
        const persisted = customStorage.getItem('ht_manager_requests');
        if (persisted) {
    ...
    const unsubscribeAuth = auth.onAuthStateChanged((user) => {
      ...
      if (user) {
        ...
        unsub = onSnapshot(collection(db, 'gmq'), (snapshot) => {
          ...
          items.sort((a, b) => b.id.localeCompare(a.id));
          setGmqList(items);
          customStorage.setItem('ht_gmq', JSON.stringify(items));
        }, (error) => {
          handleFirestoreError(error, OperationType.LIST, 'gmq');
        });
      } else {
        // Unauthenticated guests read and write to their own local GMQueue list in Firestore
        try {
          const persistedG = customStorage.getItem('ht_gmq');
          if (persistedG) {
    ...
    const unsub = onSnapshot(collection(db, 'gallery'), (snapshot) => {
      if (snapshot.empty) {
        ...
        setGalleryPhotos(prev => {
          ...
          customStorage.setItem('ht_gallery_photos', JSON.stringify(merged));
          return merged;
        });
      } else {
        const items: any[] = [];
  ...
  useEffect(() => {
    ...
    const unsubReq = onSnapshot(collection(db, 'requests'), (snapshot) => {
      ...
      customStorage.setItem('ht_requests', JSON.stringify(items));
    }, (error) => {
    ...
    const unsubVid = onSnapshot(collection(db, 'videos'), (snapshot) => {
      ...
      customStorage.setItem('ht_videos', JSON.stringify(items));
    }, (error) => {
      ...
      snapshot.forEach((docSnap) => {
        ...
        if (id === 'hero_photo') {
          if (data.url) {
            ...
            customStorage.setItem('ht_opening_photo', data.url);
          }
        } else if (id === 'live_embed') {
          if (data.url) {
            setLiveEmbedUrl(data.url);
            customStorage.setItem('ht_live_embed', data.url);
          }
        } else if (id === 'contacts') {
          setContacts(data);
          customStorage.setItem('ht_contacts', JSON.stringify(data));
        }
      });
    }, (error) => {
  ...
  const saveCartToStorage = (updatedCart: any) => {
    setCart(updatedCart);
    customStorage.setItem('ht_cart', JSON.stringify(updatedCart));
  };

  const saveSolarCartToStorage = (updatedSolarCart: any) => {
    setSolarCart(updatedSolarCart);
    customStorage.setItem('ht_solar_cart', JSON.stringify(updatedSolarCart));
  };
  ...
  const handleUpdateProducts = async (updatedProds: Product[]) => {
    ...
    customStorage.setItem('ht_products', JSON.stringify(updatedProds));
  ...
  const handleUpdateSolarProducts = async (updatedSolars: SolarProduct[]) => {
    ...
    customStorage.setItem('ht_solar_products', JSON.stringify(updatedSolars));
  ...
  const handleUpdateRepairs = async (updatedRepairs: RepairRecord[]) => {
    ...
    customStorage.setItem('ht_repairs', JSON.stringify(updatedRepairs));
  ...
  const handleUpdateGmq = async (updatedGmq: GMRequest[]) => {
    ...
    customStorage.setItem('ht_gmq', JSON.stringify(updatedGmq));
  ...
  const handleUpdateBankAccount = (account: typeof bankAccount) => {
    ...
    customStorage.setItem('ht_bank_acc', JSON.stringify(account));
  };

  const handleAddCustomDeal = (deal: Deal) => {
    const updated = [deal, ...dealsList];
    setDealsList(updated);
    customStorage.setItem('ht_deals', JSON.stringify(updated));
  };
  ...
  const handleCreateRequest = async (type: string, payload: any, note: string) => {
    ...
    customStorage.setItem('ht_requests', JSON.stringify(updated));
  ...
  const handleApproveRequest = async (id: string) => {
    ...
    customStorage.setItem('ht_requests', JSON.stringify(updatedRequests));
  ...
  const handleUpdateOpeningPhoto = async (url: string) => {
    ...
    customStorage.setItem('ht_opening_photo', url);
    try {
  ...
  const handleUpdateLiveEmbedUrl = async (url: string) => {
    setLiveEmbedUrl(url);
    customStorage.setItem('ht_live_embed', url);
    try {
  ...
  const handleUpdateContacts = async (newContacts: typeof contacts) => {
    ...
    customStorage.setItem('ht_contacts', JSON.stringify(newContacts));
    try {
...
const DEFAULT_HARDCODED_CSV = [
  ...
  const handleSaveDisplayFloor = async (activeLayout: string, savedConfigs: { [name: string]: string }) => {
    ...
    customStorage.setItem('ht_display_floor_active', activeLayout);
    customStorage.setItem('ht_display_floor_saved', JSON.stringify(savedConfigs));
    try {
  ...
  const applyPreset = async (name: string) => {
    ...
    if (name === "Default") {
      ...
      customStorage.setItem('ht_spreadsheet_rows', JSON.stringify(DEFAULT_HARDCODED_CSV));
  ...
  const handleAddVideo = async (vid: { url: string; title: string; desc?: string }) => {
    ...
    customStorage.setItem('ht_videos', JSON.stringify(updated));
...
Message: ${quickMessageText}`;
  ...
  const handleSubmitGMRequest = async () => {
    ...
    setGmqList(nextQ);
    customStorage.setItem('ht_gmq', JSON.stringify(nextQ));
  ...
  const handlePostReview = async () => {
    ...
    customStorage.setItem('ht_rev', JSON.stringify(nextReviews));
  ...
  const handleDeleteEscalationTicket = async (ticketId: string) => {
    if (window.confirm(`⚠️ Are you sure you want to permanently delete Escalation Ticket #${ticketId}?`)) {
      try {
        ...
        customStorage.setItem('ht_escalations', JSON.stringify(nextList));
  ...
  const handleDeleteManagerRequest = async (ticketId: string) => {
    if (window.confirm(`⚠️ Are you sure you want to permanently delete Manager Ticket #${ticketId}?`)) {
      try {
        ...
        setMgrTrackedTicket(null);
        customStorage.setItem('ht_manager_requests', JSON.stringify(nextList));
                ...
                if (window.confirm("Are you sure you want to remove this product image from the showcase?")) {
                  ...
                  if (code) {
                    ...
                    if (matchedItem) {
                      ...
                      customStorage.setItem('ht_img_cache', JSON.stringify(nextCache));
            ...
            {currentRoom === 'staff' && (
              <StaffRoom 
                ...
                onToggleMgrStatus={() => {
                  ...
                  setMgrStatus(val);
                  customStorage.setItem('ht_mgrstatus', val);
                }}
            ...
            {currentRoom === 'spreadsheet_manager' && (
              (() => {
                ...
                const handleAddNewProductRow = () => {
                  ...
                  customStorage.setItem('ht_spreadsheet_rows', JSON.stringify(updated));
